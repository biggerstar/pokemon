...
export async function processOrderPlaceOrder() {
  ...
  try {
    ...
    console.error(
      '[下单] 获取开发模式配置失败，为了安全起见，不进行下单:',
      error,
    );
    await TaskManager.error(
      '[下单] 无法获取开发模式配置，为了安全起见，已取消下单操作',
    );
    return;
  }
  ...
  const submitOrderButton = querySelector(
    '.list02.next-step-button a',
  ) as HTMLElement;
  if (submitOrderButton) {
    await automation
      .scrollTo(submitOrderButton)
      .move(submitOrderButton, 1000)
      .run();
    // 如果是开发模式，不进行实际下单
    ...
    await automation.click(submitOrderButton).run();
